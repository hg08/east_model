...
if __name__=='__main__':
    ...
    # Plot
    corr0 = np.load('../data/corr_EJ_beta{:4.2f}_step{:d}_h{:4.2f}.npy'.format(beta_list[0],step,h))     
    corr_arr = np.zeros((len(beta_list),corr0.shape[0]))
    for i,term in enumerate(beta_list):
        temp = np.load('../data/corr_EJ_beta{:4.2f}_step{:d}_h{:4.2f}.npy'.format(term,step,h))     
        corr_arr[i] = temp
